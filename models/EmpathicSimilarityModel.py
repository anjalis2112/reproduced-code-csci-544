...
from torchmetrics import SpearmanCorrCoef, F1Score, Accuracy, Precision, Recall, PearsonCorrCoef
...
class EmpathicSimilarityModel(pl.LightningModule):
    ...
    def eval_step(self, batch, batch_idx, prefix):
        ...
        self.pearson_corr = self.pearson_corr.to(self.device)
        
        
        f1 = self.f1_score(cos_sim,score)
        spearman = self.spearman(cos_sim.float(), batch[2])
        accuracy = self.accuracy(cos_sim, score)
        precision = self.precision(cos_sim, score)
        recall = self.recall(cos_sim, score)
        pearson_corr = self.pearson_corr(cos_sim, score)
        ...
        self.log(prefix+"_pearson_corr", pearson_corr)
